    ...
    try {
      const adsUrl = `${META_API_BASE}/${adAccountId}/ads?` +
        new URLSearchParams({
          ...
          fields: "id,name,campaign_id,preview_shareable_link,creative{id,thumbnail_url,image_url,object_story_spec{link_data{image_url},photo_data{url},video_data{image_url}},object_type,video_id}",
          limit: "200",
        });
        ...
        const perCampaignAds = await Promise.all(
          campaigns.slice(0, 10).map(async (campaign) => {
            const url = `${META_API_BASE}/${campaign.id}/ads?` +
              new URLSearchParams({
                access_token: accessToken,
                fields: "id,name,preview_shareable_link,creative{id,thumbnail_url,image_url,object_story_spec{link_data{image_url},photo_data{url},video_data{image_url}},object_type,video_id}",
                limit: "20",
              });
    ...
    if (pool.length === 0) {
      try {
        const acUrl = `${META_API_BASE}/${adAccountId}/adcreatives?` +
          new URLSearchParams({
            access_token: accessToken,
            fields: "id,name,thumbnail_url,image_url,object_type,video_id,object_story_spec{link_data{image_url},photo_data{url},video_data{image_url}}",
            limit: "200",
          });
