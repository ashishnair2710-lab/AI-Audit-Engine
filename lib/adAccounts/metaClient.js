...
export async function fetchMetaData(accessToken, adAccountId, opts = {}) {
  ...
  try {
    ...
    // ── Build campaign insights map first (used for ranking ads) ────────────
    ...
    // Helper: shape a raw ad object into our pool format
    function shapeAd(ad, campaignId) {
      const cre  = ad.creative || ad; // adcreatives endpoint returns creative directly
      ...
      const campMetrics = campaignInsightsMap[campaignId] || {};
      return {
        ad_id:       ad.id,
        campaign_id: campaignId,
        name:        ad.name || cre.name || ad.id,
        label:       ad.name || cre.name || ad.id,
        thumbnail,
        body,
        headline,
    ...
    // ── Path 1: /{adAccountId}/ads with creative expansion ────────────────
    let pool = [];
    try {
      const adsUrl = `${META_API_BASE}/${adAccountId}/ads?` +
        new URLSearchParams({
          access_token: accessToken,
          fields: "id,name,campaign_id,preview_shareable_link,creative{id,thumbnail_url,image_url,object_story_spec{link_data{message,name,description,image_url},photo_data{url},video_data{image_url,title,message}},object_type,video_id}",
          limit: "200",
        });
      const adsData = await fetch(adsUrl).then((r) => r.json());
      console.log("[MetaClient] /ads count:", adsData?.data?.length, "err:", adsData?.error?.message);
      pool = (adsData?.data || []).map((ad) => shapeAd(ad, ad.campaign_id));
    } catch (e) {
      console.error("[MetaClient] /ads threw:", e.message);
    }

    // ── Path 2: fetch ads per-campaign (works when account-level /ads fails) ─
    if (pool.length === 0) {
      try {
        const campaigns = campaignsData.data || [];
        const perCampaignAds = await Promise.all(
          campaigns.slice(0, 10).map(async (campaign) => {
            const url = `${META_API_BASE}/${campaign.id}/ads?` +
              new URLSearchParams({
                access_token: accessToken,
                fields: "id,name,preview_shareable_link,creative{id,thumbnail_url,image_url,object_story_spec{link_data{message,name,description,image_url},photo_data{url},video_data{image_url,title,message}},object_type,video_id}",
                limit: "20",
              });
            const d = await fetch(url).then((r) => r.json());
            return (d?.data || []).map((ad) => shapeAd(ad, campaign.id));
          })
        );
        pool = perCampaignAds.flat();
        console.log("[MetaClient] per-campaign /ads total:", pool.length);
      } catch (e) {
        console.error("[MetaClient] per-campaign threw:", e.message);
      }
    }

    // ── Path 3: /{adAccountId}/adcreatives ───────────────────────────────
    if (pool.length === 0) {
      try {
        const acUrl = `${META_API_BASE}/${adAccountId}/adcreatives?` +
          new URLSearchParams({
            access_token: accessToken,
            fields: "id,name,thumbnail_url,image_url,object_type,video_id,object_story_spec{link_data{message,name,description,image_url},photo_data{url},video_data{image_url,title,message}}",
            limit: "200",
          });
        const acData = await fetch(acUrl).then((r) => r.json());
        console.log("[MetaClient] /adcreatives count:", acData?.data?.length, "err:", acData?.error?.message);
        const firstCampaignId = campaignsData.data?.[0]?.id;
        pool = (acData?.data || []).map((cr) => shapeAd(cr, firstCampaignId));
      } catch (e) {
        console.error("[MetaClient] /adcreatives threw:", e.message);
      }
    }

    // ── Path 4: /{adAccountId}/adimages (raw uploaded images, last image path) ─
    if (pool.length === 0) {
      try {
        const aiUrl = `${META_API_BASE}/${adAccountId}/adimages?` +
          new URLSearchParams({
            access_token: accessToken,
            fields: "id,name,url,url_128,created_time",
            limit: "50",
          });
        const aiData = await fetch(aiUrl).then((r) => r.json());
        console.log("[MetaClient] /adimages count:", aiData?.data?.length, "err:", aiData?.error?.message);
        const firstCampaignId = campaignsData.data?.[0]?.id;
        pool = (aiData?.data || []).map((img) => ({
          ad_id:       img.id,
          campaign_id: firstCampaignId,
          name:        img.name || img.id,
          label:       img.name || img.id,
          thumbnail:   img.url_128 || img.url || null,
          preview_url: null,
          format:      "image",
          ...(campaignInsightsMap[firstCampaignId] || { ctr: 0, spend: 0, impressions: 0, clicks: 0, conversions: 0, cpa: null }),
        }));
      } catch (e) {
        console.error("[MetaClient] /adimages threw:", e.message);
      }
    }

    // ── Path 5: campaign-level fallback (last resort — no thumbnails) ─────
    if (pool.length === 0 && campaignsData.data?.length) {
      console.log("[MetaClient] all ad paths empty — using campaign fallback");
      pool = (campaignsData.data || []).map((campaign) => {
    ...
    const campaigns = (campaignsData.data || []).map((campaign) => {
      ...
      const campaignAds = pool.filter((ad) => ad.campaign_id === campaign.id);
