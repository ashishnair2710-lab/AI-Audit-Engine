...
export async function fetchMetaData(accessToken, adAccountId, opts = {}) {
  ...
  try {
    ...
    const adsRanked = (adSetsData?.data || []).map((ad) => {
    ...
    const top_creatives = [...adsRanked].sort((a, b) => b.ctr - a.ctr).slice(0, 3);
    const medianSpend = adsRanked.map((a) => a.spend).sort((x, y) => x - y)[Math.floor(adsRanked.length / 2)] || 0;
    const underperforming_creatives = adsRanked
      .filter((a) => a.spend >= medianSpend)
