...
export async function fetchMetaData(accessToken, adAccountId, opts = {}) {
  ...
  try {
    ...
    const adsUrl =
      ...
      new URLSearchParams({
        ...
        fields: [
          ...
          "campaign_id",
          "status",
          "creative{thumbnail_url,image_url,object_story_spec{link_data{image_url,link},photo_data{url},video_data{image_url}},object_type,video_id,effective_instagram_story_id,effective_object_story_id}",
    ...
    // Prefer ads with real performance data; fall back to all ads; last resort = campaign-level entries
    const withMetrics = adsRankedAll.filter((a) => a.impressions > 0);
    let pool = withMetrics.length ? withMetrics : adsRankedAll;

    // If the /ads endpoint returned nothing, synthesise entries from campaign data
    if (pool.length === 0 && campaignsData.data?.length) {
      pool = (campaignsData.data || []).map((campaign) => {
        ...
        return {
          ...
          thumbnail:  null,
          format:     "campaign",
          ctr, spend, impressions: impr, clicks,
          conversions: conv,
          cpa: conv > 0 ? Math.round(spend / conv) : null,
        };
