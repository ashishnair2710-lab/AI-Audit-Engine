...
export async function fetchMetaData(accessToken, adAccountId, opts = {}) {
  ...
  try {
    ...
    const fields = [
      ...
      "insights.fields(spend,impressions,clicks,ctr,actions,action_values,cpm,cpp){date_preset}",
    ].join(",");

    const baseParams = { access_token: accessToken, fields, limit: "50" };
    if (isLifetime) {
      baseParams.date_preset = "maximum";
    } else {
      const { since, until } = getDateRange(opts.days || 30);
      baseParams.time_range  = JSON.stringify({ since, until });
    }
    const campaignsUrl = `${META_API_BASE}/${adAccountId}/campaigns?` + new URLSearchParams(baseParams);
