...
export async function fetchMetaData(accessToken, adAccountId, opts = {}) {
  ...
  try {
    ...
    console.log("[MetaClient] /ads response — count:", adSetsData?.data?.length, "error:", adSetsData?.error?.message);
    ...
    // Use ads if available; fall back to campaign-level entries so the UI always shows something
    let pool = adsRankedAll;
    if (pool.length === 0 && campaignsData.data?.length) {
      console.log("[MetaClient] /ads returned nothing — falling back to campaign-level entries");
