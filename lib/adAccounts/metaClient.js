...
export async function fetchMetaData(accessToken, adAccountId, opts = {}) {
  ...
  try {
    ...
    // ── Path 2: /{adAccountId}/adcreatives (different endpoint, often works
    //    when /ads creative expansion fails on agency accounts) ────────────
    if (pool.length === 0) {
      try {
        ...
        // adcreatives don't have campaign_id — assign the first campaign's metrics as approximation
    ...
    // ── Path 3: campaign-level fallback (last resort — no thumbnails) ─────
