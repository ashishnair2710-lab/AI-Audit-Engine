...
function shapeCompetitor(brand, ads) {
  ...
  ads.forEach((ad) => {
    // Format detection from snapshot URL hint (API doesn't return media_type for free)
  ...
  return {
    ...
    ads: ads.slice(0, 8).map((a) => ({
      ...
      video_url:    "",
      format:       "image",
      is_active:    !a.ad_delivery_stop_time || new Date(a.ad_delivery_stop_time) > new Date(),
      snapshot_url: a.ad_snapshot_url,
      platforms:    a.publisher_platforms || [],
      start:        a.ad_delivery_start_time,
      stop:         a.ad_delivery_stop_time,
      impressions:  a.impressions?.lower_bound
        ? `${Number(a.impressions.lower_bound).toLocaleString()}–${Number(a.impressions.upper_bound).toLocaleString()}`
        : null,
    })),
  };
}
