...
function generateCrossInsights(metaResult, googleResult) {
  ...
  if (googleCpaIsHigh && tofuIsWeak) {
    ...
    insights.push({
      ...
      detail:   `Google CPA is ${formatCurrency(googleCpa)} and Meta TOFU spend is weak. You are relying on Google to capture demand, but without Meta building brand awareness, the audience pool is thin. Google ends up paying more for fewer conversions.`,
  ...
  if (metaEngagementStrong && metaConversionWeak) {
    insights.push({
      ...
      title:    "Engagement is Strong. Conversions Are Leaking",
      detail:   `Meta CTR is ${metaCtr}% (healthy) but only ${metaConversions} conversions were recorded despite significant spend. People are clicking but not buying. The issue is post-click: landing page, offer, or checkout friction.`,
  ...
  if (googleSpendShare > 80 && metaTotalSpend > 0) {
    insights.push({
      ...
      detail:   `${googleSpendShare}% of paid media budget is on Google, ${metaSpendShare}% on Meta. Google captures existing demand. Without Meta building new demand, growth will plateau as your addressable market shrinks.`,
