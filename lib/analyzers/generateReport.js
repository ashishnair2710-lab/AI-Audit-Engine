...
function generateReport(metaResult, googleResult, competitorResult, crossInsights, scoreResult) {
  ...
  return {
    ...
    score_color_hex:     scoreResult.color_hex,
    score_verdict:       scoreResult.label,
    score_mode:          scoreResult.mode,
    score_platforms:     scoreResult.platforms,
    ...
    meta_audit: {
      ...
      top_creatives:             metaResult?.top_creatives             || [],
      underperforming_creatives: metaResult?.underperforming_creatives || [],
    },
    ...
    google_audit: {
      ...
      top_campaigns:    googleResult?.top_campaigns    || [],
      worst_campaigns:  googleResult?.worst_campaigns  || [],
    },
...
function buildGrowthOpportunities(meta, google, competitor, score) {
  ...
  if (!meta?.signal_quality?.capi_enabled) {
    opportunities.push({
      ...
      detail:   "Post-iOS 14, browser-only tracking misses ~30% of conversions. Enabling Conversions API recovers this signal, giving Meta's algorithm better data to optimise. Typically improves ROAS by 15-25%.",
...
function buildSummary(meta, google, competitor, score, crossInsights) {
  ...
  if (wastedSpend > 0) {
    summaryLines.push(
      `${formatCurrency(wastedSpend)}/month is being wasted on Google campaigns with zero return. Stop or restructure these campaigns today.`
    );
  }

  if (pixelOff) {
    summaryLines.push(
      "The Meta Pixel is inactive. Every dollar spent on Meta is being delivered without optimisation data. CPAs will be significantly higher until this is resolved."
    );
  }
