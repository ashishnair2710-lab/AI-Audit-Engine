...
function AdPostCard({ c, tone }) {
  ...
  const copy = c.body || c.headline || null;

  return (
    <div className="flex items-start gap-3 p-3 rounded-xl border border-brand-border bg-white hover:border-gray-300 transition-colors">
      {/* Thumbnail */}
      <div className="flex-shrink-0 w-14 h-14 rounded-lg overflow-hidden bg-[#F0F0F0] border border-brand-border relative">
        ...
        <span className="absolute bottom-0.5 left-0.5 text-[7px] font-bold uppercase bg-black/50 text-white px-1 rounded leading-4">
          {format}
        </span>
      </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {copy && (
          <p className="text-[11px] text-brand-subtext mt-0.5 leading-snug line-clamp-2">{copy}</p>
        )}
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          {c.preview_url && (
            <a href={c.preview_url} target="_blank" rel="noopener noreferrer"
              className="text-[10px] text-brand-purple font-semibold hover:underline ml-auto">
              View ↗
            </a>
          )}
        </div>
